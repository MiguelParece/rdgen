...
def gen_protocol_handler_component(app_name):
    def func(lines, index_start):
        indent = g_indent_unit * 3
        # Create a registry-only component that registers the eoconnect: protocol.
        # KeyPath is set on the default HKCR eoconnect value so MSI owns/uninstalls it.
        # Use the same install dir/exe naming convention as other entries: [INSTALLFOLDER_INNER]{app_name}.exe
        to_insert = f"""
{indent}<Component Guid="{uuid.uuid4()}">
{indent}{g_indent_unit}<RegistryValue Root="HKCR" Key="eoconnect" Value="URL:EO.Connect Protocol" Type="string" KeyPath="yes" />
{indent}{g_indent_unit}<RegistryValue Root="HKCR" Key="eoconnect" Name="URL Protocol" Value="" Type="string" />
{indent}{g_indent_unit}<RegistryValue Root="HKCR" Key="eoconnect\\shell\\open\\command" Value="[INSTALLFOLDER_INNER]{app_name}.exe &quot;%1&quot;" Type="string" />
{indent}</Component>
"""
        lines.insert(index_start + 1, to_insert[1:])
        return lines
